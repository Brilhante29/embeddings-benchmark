...
- Fastest source run: BGE-small at median `3.0561 ms/query`
...
Both dense models recovered the same relevant documents. BGE-small was `4.93x` faster on the measured end-to-end query path, while MiniLM had lower cold indexing time in this run. Timing is hardware-specific; comparisons require the same workload key and Docker runtime.
