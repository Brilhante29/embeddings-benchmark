...
The Docker build preloads both model artifacts. The benchmark then runs as an unprivileged user with networking disabled. V2 evidence at `benchmarks/publication/embeddings-baseline-v2.json` binds the source commit, image digest, fixtures, benchmark config, Linux dependency lock, and raw JSON.
